    ...
    if state == STATE_IDLE:
        if should_check_api():
            ...
            if signal is None:
                ...
                if not positions:
                    log("[IDLE] Ordem enviada, mas posição ainda não confirmada no MT5")
                    continue
    ...
    # =================================================
        ...
        if should_check_api():
            ...
            if signal == "NEUTRAL":
                ...
                if positions:
                    log("[Neutro] Ordem enviada, mas posição ainda não confirmada no MT5")
                    continue
            ...
            # 3️⃣ SINAL OPOSTO → reversão imediata
                ...
                while True:
                    time.sleep(0.3)
                    if not mt5.positions_get(symbol=tick_demo):
                       break
                
                if not market_is_open(tick_demo):
                    log("[Reversão] Mercado fechado — aguardando abertura")
                    time.sleep(5)
                    continue
                ...
                if result is None:
                    log("[Reversão] Ordem rejeitada — permanecendo fora")
                    force_api_check = True
                    time.sleep(5)
                    continue
    ...
    # =================================================
        ...
        if pos.type == mt5.POSITION_TYPE_BUY:
            ...
            if price <= stop_low:
                ...
                positions = mt5.positions_get(symbol=tick_demo)
                if positions:
                    log("[STOP BUY] Fechamento pendente - aguardando confirmação MT5 ")
        ...
        # =========================
            ...
            if price >= stop_high:
                ...
                positions = mt5.positions_get(symbol=tick_demo)
                if positions:
                    log("[STOP SELL] Fechamento pendente - aguardando confirmação MT5 ")
    ...
    # =================================================
        ...
        if should_check_api():
            ...
            # 3️⃣ SINAL OPOSTO → ENTRA IMEDIATAMENTE
                ...
                if not positions:
                    log('[POST_STOP] Ordem enviada, mas posição não confirmada - aguardando')
